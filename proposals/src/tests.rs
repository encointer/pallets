...
use encointer_primitives::{
	...
	proposals::{ProposalAccessPolicy, ProposalAction, ProposalState},
};
use frame_support::{assert_err, assert_ok};
use mock::{new_test_ext, EncointerProposals, Origin, System, TestRuntime};
...
fn proposal_submission_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(EncointerProposals::submit_proposal(Origin::signed(alice()), proposal.clone()));
		assert_eq!(EncointerProposals::proposal_count(), 1);
		assert_eq!(EncointerProposals::proposals(1).unwrap(), proposal);
	});
}
