...
impl<T: Config> Module<T> {
	...
	fn assign_meetups() {
		...
		for cid in cids.iter() {
			...
			if !meetups.is_empty() {
				let mut location_assignments: Vec<MeetupLocationIndexType> =
					(1..=n_locations as MeetupLocationIndexType).collect();
				location_assignments =
					location_assignments.random_permutation(&mut random_source).unwrap_or_default();
				...
				for (i, m) in meetups.iter().enumerate() {
					for p in meetups[i].iter() {
						<MeetupLocationIndex<T>>::insert(
							(cid, cindex),
							p,
							&location_assignments[i],
						);
					}
					<MeetupRegistry<T>>::insert((cid, cindex), &location_assignments[i], m.clone());
				}
			};
			debug!(target: LOG, "assigned {} meetups for cid {:?}", meetups.len(), cid);
		}
	...
	fn issue_rewards() {
		...
		for cid in cids.iter() {
			...
			for m in MeetupLocationIndex::<T>::iter_prefix_values((cid, cindex))
				.collect::<BTreeSet<MeetupLocationIndexType>>()
				.iter()
			{
				...
				let (n_confirmed, n_honest_participants) =
					match Self::ballot_meetup_n_votes(cid, cindex, *m) {
						...
						_ => {
							warn!(
								target: LOG,
								"ignoring meetup {} because votes are not dependable", m
							);
							continue
						},
					};
				...
				for p in &meetup_participants {
					if Self::meetup_participant_count_vote((cid, cindex), &p) != n_confirmed {
						debug!(
							target: LOG,
							"skipped participant because of wrong participant count vote: {:?}", p
						);
						continue
					}
					...
					if attestees.len() < (n_honest_participants - 1) as usize {
						debug!(
							target: LOG,
							"skipped participant because didn't testify for honest peers: {:?}", p
						);
						continue
					}
	...
	pub fn get_meetup_location(
		...
		if (meetup_location_idx > 0) &&
			(meetup_location_idx <= locations.len() as MeetupLocationIndexType)
		{
