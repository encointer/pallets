...
fn new_community_works() {
	new_test_ext().execute_with(|| {
		...
		let community_meta: BoundedCommunityMetadata = BoundedCommunityMetadata {
		...
		let cid = CommunityIdentifier::new(location, bs.clone()).unwrap();
...
fn two_communities_in_same_bucket_works() {
	new_test_ext().execute_with(|| {
		...
		let cid2 = CommunityIdentifier::new(location2, bs2).unwrap();
		let cids = EncointerCommunities::community_identifiers();
...
fn updating_community_metadata_works() {
	new_test_ext().execute_with(|| {
		...
		let new_metadata = BoundedCommunityMetadata {
...
fn add_location_works() {
	new_test_ext().execute_with(|| {
		...
		EncointerCommunities::add_location(
			RuntimeOrigin::signed(some_bootstrapper),
			cid,
			location3,
		)
		.ok();
