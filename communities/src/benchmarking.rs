...
benchmarks! {
    ...
    // spread locations along the equator, ie. lat = 0, lon equally spread over [-170, 170]
    // using 170 to have a save distance from the dateline
    let locations: Vec<Location> = (0..NUM_LOCATIONS)
    .map(|x| Location{lat: Degree::from_num(0.0), lon: Degree::from_num(((x as f64 * 340.0) / (NUM_LOCATIONS as f64)) - 170.0)})
    .collect();
