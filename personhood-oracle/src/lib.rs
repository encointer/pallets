...
use encointer_primitives::{
	...
	sybil::{
		sibling_junction, CallMetadata, OpaqueRequest, PersonhoodUniquenessRating, RequestHash,
		SybilResponseCall,
	},
};
	...
	pub fn verify(
		...
		for proof in request.iter() {
				...
				.contains(&proof.community_identifier)
			{
				warn!(
					target: LOG,
					"Received ProofOfAttendance for unknown cid: {:?}", proof.community_identifier
				);
				continue
			}
		...
		Ok(PersonhoodUniquenessRating::new(n_attested, last_n_ceremonies, attested))
	}
